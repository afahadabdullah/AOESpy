...
def plot(var,lon=[],lat=[],title='',clf=[],cl=[], cmap='coolwarm',lon1=-180,lon2=180,lat1=-90,lat2=90,bar=1,p=1,m=1,lmask=0, alpha=.95):
    ...
    if nanmax(lat)-nanmin(lat)>89:
        parallels = arange(-90,90+1,30)

    if nanmax(lon)-nanmin(lon)>179:
    ...
    if len(cl)==1:
        ...
        if len(clf)>1:
            csf = map.contourf(x,y,var,clf,extend='both',cmap=cmap, alpha=alpha)
            if bar==1:
                cb = map.colorbar(csf,"bottom", extend='both',size="3%", pad="12%")
            cs = map.contour(x,y,var,cl,colors='k',linewidths=0.3, alpha=alpha)
            #plt.clabel(cs, inline=True, fmt='%1.1f', fontsize=6, colors='k')
            plt.title(title,fontsize=9) 
        else:
            csf = map.contourf(x,y,var,extend='both',cmap=cmap, alpha=alpha)
            #cb = map.colorbar(csf,"bottom", extend='both',size="3%", pad="9%")
            if bar==1:
                cb = map.colorbar(csf,"bottom", extend='both',size="3%", pad="12%")
            cs = map.contour(x,y,var,colors='k',linewidths=0.3, alpha=alpha)
            #plt.clabel(cs, inline=True, fmt='%1.1f', fontsize=6, colors='k')
            plt.title(title,fontsize=9)
    else:
